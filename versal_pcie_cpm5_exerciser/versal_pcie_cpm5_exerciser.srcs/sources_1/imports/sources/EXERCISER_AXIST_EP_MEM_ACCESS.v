...
`define EXERCISER_AXIST_MEM_ACCESS_WR_RST     3'b000
`define EXERCISER_AXIST_MEM_ACCESS_WR_WAIT    3'b001
`define EXERCISER_AXIST_MEM_ACCESS_WR_READ    3'b010
`define EXERCISER_AXIST_MEM_ACCESS_WR_WRITE   3'b100

(* DowngradeIPIdentifiedWarnings = "yes" *)
module EXERCISER_AXIST_EP_MEM_ACCESS #(
   ...
   input [3:0]	    cfg_neg_max_link_width,     // I [3:0]
   ...
   // Read Access
   // Read Port
   input [6:0]	    addr_i,      // I [6:0]
   input [3:0]	    rd_be_i,     // I [3:0]
   output [31:0]    rd_data_o,     // O [31:0]

   // Write Access
   // Write Port
   input [7:0]	    wr_be_i,                // I [7:0]
   input [31:0]     wr_data_i,              // I [31:0]
   input            wr_en_i,                // I
   output           wr_busy_o,              // O

   output           init_rst_o,             // O

   output           mrd_start_o,            // O
   output           mrd_int_dis_o,          // O
   output           mrd_done_o,             // O
   output [31:0]    mrd_addr_o,             // O [31:0]
   output [15:0]    mrd_len_o,              // O [31:0]
   output [2:0]     mrd_tlp_tc_o,           // O [31:0]
   output           mrd_64b_en_o,           // I
   output           mrd_phant_func_dis1_o,  // O [2:0]
   output [7:0]     mrd_up_addr_o,          // O
   output [15:0]    mrd_count_o,
   input            mrd_done_i,             // O [7:0]
   output           mrd_relaxed_order_o,
   output           mrd_nosnoop_o,
   output [7:0]     mrd_wrr_cnt_o,

   output           mwr_start_o,            // O
   output           mwr_int_dis_o,          // O
   input            mwr_done_i,             // I
   output [31:0]    mwr_addr_o,             // O [31:0]
   output [15:0]    mwr_len_o,              // O [31:0]
   output [15:0]    mwr_count_o,            // O [31:0]
   output [31:0]    mwr_data_o,             // O [31:0]
   output [2:0]     mwr_tlp_tc_o,           // O [2:0]
   output           mwr_64b_en_o,           // O
   output           mwr_phant_func_dis1_o,
   output [7:0]     mwr_up_addr_o,          // O [7:0]
   ...
   output [31:0]    cpld_data_o,            // O [31:0]
   input [31:0]     cpld_found_i,           // I [31:0]
   input [31:0]     cpld_data_size_i,       // I [31:0]
   input            cpld_malformed_i,       // I
   input            cpld_data_err_i,        // I

   output           rd_metering_o,          // O
   input            tags_all_back_i,

   output [31:0]     cfg_err_cor_o,         // O [31:0]
   output [31:0]     aer_err_cor_o,         // O [31:0]
   output [31:0]     aer_err_uncor_o,       // O [31:0]
   output [31:0]     aer_control_o,         // O [31:0]
   //gp_data_upstream,                      // I
   //gp_data_dnstream,                      // O
...
(* mark_debug *) wire         wr_busy_dbg      = wr_busy_o;
   ...
   always @(posedge clk ) begin
      ...
      if ( !rst_n ) begin
         ...
         wr_mem_state <= `EXERCISER_AXIST_MEM_ACCESS_WR_RST;

      end else begin

         case ( wr_mem_state )
           `EXERCISER_AXIST_MEM_ACCESS_WR_RST : begin
              mem_write_en <= 1'b0;
              if (wr_en_i) begin // read state
                 wr_mem_state <= `EXERCISER_AXIST_MEM_ACCESS_WR_READ ;
              end else begin
                 wr_mem_state <= `EXERCISER_AXIST_MEM_ACCESS_WR_RST;
              end
           end
           `EXERCISER_AXIST_MEM_ACCESS_WR_READ : begin
              mem_write_en <= 1'b0;
              pre_wr_data  <= mem_rd_data;
              wr_mem_state <= `EXERCISER_AXIST_MEM_ACCESS_WR_WRITE;
           end
           `EXERCISER_AXIST_MEM_ACCESS_WR_WRITE : begin
    ...
    * Write controller busy
     */
    assign wr_busy_o = wr_en_i | (wr_mem_state != `EXERCISER_AXIST_MEM_ACCESS_WR_RST);
...
EXERCISER_AXIST_EP_MEM #(
    .NUM_GP_PORTS(NUM_GP_PORTS)
) EP_MEM (
   ...
   .cfg_err_cor_o             (cfg_err_cor_o),         // O [15:0]
